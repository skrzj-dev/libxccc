...
/* --- */
#include "_xcvector_ds_api_u.hpp"
/* --- */
...
static int verifyInternalState_itemsIntegrity_entry(xc_tcVectorS_LL_inspect_LLII_info_t* tmp_info, const int public_idx, xc_tcVectorS_hdr_t* refp_hdr, TPL_VECTOR_TYPE* refp_self);
...
static int verifyInternalState_itemsIntegrity_entry(xc_tcVectorS_LL_inspect_LLII_info_t* tmp_info, const int public_idx, xc_tcVectorS_hdr_t* refp_hdr, TPL_VECTOR_TYPE* refp_self)
{
	...
	do
	{
		...
		if(1)
		{
			...
			xcc_test2_expect( tmp_info->byteptr_item == (xc_byteptr_t)&refp_self->items_array_ptr[public_idx] );
		}
		/* : done */
...
{
	xc_tcVectorS_LL_deref_t deref={0};
	xc_tcVectorS_hdr_t* refp_hdr=NULL;
	int public_idx=0;
	xcc_test2_scope_disconnected();
	...
	deref=xc_tcVectorS_u_derefer_get_retv(refp_self);
	...
	do
	{
		...
		{
			xc_tcVectorS_LL_inspect_LLII_info_t tmp_info={0};
			/* --- */
			
			// log idx
			
			xcc_test2_expect( 0 == xc_tcVectorS_LL_inspect_LLII_info_cp(deref, public_idx, &tmp_info) );
...
testIntf_internal_xcVectorDS__method(int)  verifyInternalState_itemsValues(TPL_VTYPE* refp_self, const int expected_capacity, const int expected_length, std::vector<_ARG_TPL_VITEMTYPE_>& arg_refp_expected_item_order) noexcept
{
	...
	do
	{
		...
		for(n=refp_self->prv__hdrDS.runtime.cur_length; n<refp_self->prv__hdrDS.runtime.cur_capacity; n++)
		{
...
testIntf_internal_xcVectorDS__method(int) hack_forceSmallerCapacity(TPL_VTYPE* refp_self,  const size_t expected_static_capacity) noexcept
{
	...
	try
	{
		...
		if( 0 != xc_tcVectorS_u_init(refp_self, expected_static_capacity) ) {
			xcc_err_term_unmg();
		}
