...
xcc_tplTestVector_static_method(int) test_rem_idx(xcc_test2_param_list) noexcept
{
	xcc_test2_scope("");
	...
	do
	{
		...
		if(1) 
		{
			if(1)
			{
				const size_t expected_capacity=vector_I.DIAG_get_expected_capacity_for_length(8);
	...
	do
	{
		...
		if(1) 
		{
			...
			if(1)
			{
				xcc_test2_expect( 0 == vector_I.remove_idx(&vector_obj, 4) );
			...
			if(1)
			{
				xcc_test2_expect( 0 == vector_I.remove_idx(&vector_obj, 2) );
