...
static int verifyInternalState_itemsIntegrity_entry(xc_tcPoolS_LL_inspect_LLII_info_t* tmp_info, const int public_idx, xc_tcPoolS_hdr_t* refp_hdr, TPL_VECTOR_TYPE* refp_self);
...
static int verifyInternalState_itemsIntegrity_entry(xc_tcPoolS_LL_inspect_LLII_info_t* tmp_info, const int public_idx, xc_tcPoolS_hdr_t* refp_hdr, TPL_VECTOR_TYPE* refp_self)
{
	...
	do
	{
		...
		if(1)
		{
			xcc_test2_expect( tmp_info->anyptr_item == (void*)&refp_self->arrayDS_ptr_items[public_idx] );
			xcc_test2_expect( tmp_info->byteptr_item == (xc_byteptr_t)&refp_self->arrayDS_ptr_items[public_idx] );
		}
		/* : done */
	...
	do
	{
		...
		if(1)
		{
			xc_byteptr_t byteptr_items=(xc_byteptr_t)refp_self->arrayDS_ptr_items;
			void* anyptr_items=(void*)refp_self->arrayDS_ptr_items;
...
testIntf_internal_xcVectorDS__method(int) verifyInternalState_itemsNumbers(TPL_VTYPE* refp_self, const int expected_capacity, const int expected_length) noexcept
{
...
testIntf_internal_xcVectorDS__hdr()
testIntf_internal_xcVectorDS__method(int) verifyInternalState_itemsIntegrity(TPL_VTYPE* refp_self, const int expected_capacity, const int expected_length) noexcept
{
	...
	deref=xc_tcPoolS_u_derefer_get_retv(refp_self);
	...
	do
	{
		...
		{
			xc_tcPoolS_LL_inspect_LLII_info_t tmp_info={0};
			/* --- */
			
			// log idx
			
			xcc_test2_expect( 0 == xc_tcPoolS_LL_inspect_LLII_info_cp(deref, public_idx, &tmp_info) );
...
testIntf_internal_xcVectorDS__hdr()
testIntf_internal_xcVectorDS__method(int)  verifyInternalState_itemsValues(TPL_VTYPE* refp_self, const int expected_capacity, const int expected_length, std::vector<_ARG_TPL_VITEMTYPE_>& arg_refp_expected_item_order) noexcept
{
	std::vector<_ARG_TPL_VITEMTYPE_>& expected_items=arg_refp_expected_item_order;
	size_t n=0;
	xcc_test2_scope_disconnected();
	...
	do
	{
		...
		for(n=0; n<max; n++)
		{
			...
			if(1)
			{
				refp_cur_item=&refp_self->arrayDS_ptr_items[ tmp_free_idx.idx0 ];
			}
...
testIntf_internal_xcVectorDS__hdr()
testIntf_internal_xcVectorDS__method(int)  verifyInternalState_2vectors_same(TPL_VTYPE* refp_self, const TPL_VTYPE* refp_2nd) noexcept
{
	size_t n=0;
	xcc_test2_scope_disconnected();
...
testIntf_internal_xcVectorDS__hdr()
testIntf_internal_xcVectorDS__method(int) hack_forceSmallerCapacity(TPL_VTYPE* refp_self,  const size_t expected_static_capacity) noexcept
{
	xcc_err_decl();
	...
	try
	{
		...
		if( expected_static_capacity > refp_hdr->runtime.cur_capacity ) {
			xcc_err_term_unmg();
		}
		
		if( 0 != xc_tcPoolS_u_init(refp_self, expected_static_capacity) ) {
			xcc_err_term_unmg();
		}
