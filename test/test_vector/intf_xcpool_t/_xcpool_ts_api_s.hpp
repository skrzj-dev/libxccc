...
#ifndef _XCC_TEST_TPL_VECTOR_API_CPOOL_TS_HPP_
#define _XCC_TEST_TPL_VECTOR_API_CPOOL_TS_HPP_
...
#include "xc/tcpool_t/xc_tcpool_t_api_s.h"
/* --- */
...
class intf_api_s: public tplTestVector_vectorIntf<TPL_VTYPE, _ARG_TPL_VITEMTYPE_, ARG_TPL_DEREFER>
{
	...
	public: ARG_TPL_DEREFER derefer_retv(TPL_VTYPE* refp_self) noexcept
	{
		...
		retv=xc_tcPoolT_s_derefer_get_retv(refp_self);
	...
	{
		return xc_tcPoolT_s_remove_idx(
			  refp_self
			, idx
		);
	}
	...
	{
		return xc_tcPoolT_s_item_direct(
			  refp_self
			, idx
		);
	}
	...
	{
		return xc_tcPoolT_s_get_length(
			  refp_self
		);
	}
	...
	{
		return xc_tcPoolT_s_get_capacity(
			  refp_self
		);
	}	
	...
	public: TPL_VTYPE* alloc_init(TPL_VTYPE* refp_self) noexcept
	{
		...
		return xc_tcPoolT_s_alloc_init(
			  refp_self
		);
	...
	{
		return xc_tcPoolT_s_assign_from(
			  refp_self
			, refp_self_src
			, result_overflow
		);
